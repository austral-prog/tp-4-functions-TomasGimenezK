...
def total_letters(text):
    ...
    """
    aloha = count_consonants(text) + count_vowels(text)
    return aloha
...
def vowel_percentage(text):
    ...
    if total_letters(text) != 0:
        porcentaje = round(count_vowels(text) / total_letters(text) * 100, 1)
        return porcentaje
    else:
        return 0.0
...
def analyze_text(text):
    ...
    chau = f"V:{count_vowels(text)} C:{count_consonants(text)} T:{total_letters(text)} P:{vowel_percentage(text)}%"
    return chau
